...
{ 
    ...
    while(get_size(blocknode))
    {
        ...
        printf("header:%lu, allocate condition: %d,size:%lu, footer:%lu, block:%lu-->",
            (word_t)blocknode->header, (int)get_alloc(blocknode),(word_t)get_size(blocknode), (word_t)footer, (word_t)blocknode);
